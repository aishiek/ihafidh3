...
// Types
type VerseCache = Map<string, Verse[]>;

// Cache
const verseCache: VerseCache = new Map();
const cacheAccessOrder: string[] = [];

// Cache helpers
const getCacheKey = (surahId: number, page: number, language: string): string => {
  return `surah_${surahId}_page_${page}_lang_${language}`;
};
...
const getCachedSurahVerses = (surahId: number, language: string) => {
  const cacheKey = getCacheKey(surahId, 1, language);
...
const clearSurahCacheById = (surahId: number) => {
  for (const key of verseCache.keys()) {
    if (key.startsWith(`surah_${surahId}_`)) {
...
const getDynamicBatchSize = (surahVerseCount: number, isInitial: boolean): number => {
  if (isInitial) return Math.min(30, Math.max(10, Math.ceil(surahVerseCount / 3)));
  return 10;
};
...
export default function ReadScreen() {
  ...
  const surahLoadingState = useRef<Map<string, { currentPage: number; hasMore: boolean }>>(new Map());
  ...
  const loadInitialVerses = useCallback(
    async (surah: Surah) => {
      ...
      const loadPromise = (async () => {
        ...
        try {
          ...
          const savedState = surahLoadingState.current.get(`surah_${surah.id}_${translationLanguage}`);

          if (cachedData) {
            setVerses(cachedData);
            setLastReadVerse(cachedData[0]);
            if (savedState) {
              setCurrentPage(savedState.currentPage);
              setHasMoreVerses(savedState.hasMore);
            } else {
              setCurrentPage(1);
              setHasMoreVerses(cachedData.length < surah.versesCount);
            }
          } else {
            setCurrentPage(1);
            ...
            try {
              ...
              if (langBase !== 'en') {
                const translations = await fetchTranslationsForVerses(
                  surah.id,
                  mappedVerses.map((m: any) => ({ verse_number: m.verseNumber })),
                  translationLanguage
                );
                mappedVerses = mappedVerses.map((mv, idx) => ({
                  ...mv,
                  translation: translations[idx] || mv.translation || '',
                }));
              }
              ...
              surahLoadingState.current.set(`surah_${surah.id}_${translationLanguage}`, {
                currentPage: 1,
                hasMore: mappedVerses.length > batchSize,
              });
              (versesRef as any).currentFullSurah = mappedVerses;
            } catch (err) {
              setLoadingError(`Failed to load verses for ${surah.name}`);
            }
          }
        } catch (err) {
          setLoadingError(`Failed to load verses for ${selectedSurah?.name}`);
          setVerses([]);
        } finally {
          setIsLoading(false);
        }
    ...
    [translationLanguage, setLastReadVerse, selectedSurah?.name]
  );
  ...
  const loadMoreVerses = useCallback(async (): Promise<Verse[]> => {
    ...
    try {
      ...
      if (verseCache.has(cacheKey)) {
        ...
        fetchedVerses = Array.isArray(result) ? result : result.verses || [];
      ...
      if (fetchedVerses.length > 0) {
        setVerses((prev) => {
          ...
          const updatedVerses = [...prev, ...newVerses];
          versesRef.current = updatedVerses;
          return updatedVerses;
        });
        setCurrentPage(nextPage);
        const stillHasMore = (versesRef.current?.length || 0) < totalVersesInSurah;
        setHasMoreVerses(stillHasMore);
      }
      return fetchedVerses;
    } catch (error) {
      setLoadingError('Failed to load more verses. Please try again.');
      return [];
    } finally {
      setIsLoadingMore(false);
    }
  }, [selectedSurah, isLoadingMore, hasMoreVerses, currentPage, translationLanguage, totalVersesInSurah]);

  const handleLoadMore = useCallback(() => {
    if (!isLoadingMore && hasMoreVerses) loadMoreVerses();
  }, [isLoadingMore, hasMoreVerses, loadMoreVerses]);
  ...
  const renderFooter = () => (
    isLoadingMore ? (
      <View style={styles.footerLoader}>
        ...
        <Text style={styles.loadingMoreText}>Loading more verses...</Text>
      </View>
    ) : null
  );
  ...
  useEffect(() => {
    if (lastViewedSurahId && !selectedSurah && !suppressNextAutoOpen.current) {
  ...
  useEffect(() => {
    ...
    if ((needsTajweed && !hasTajweed) || (!needsTajweed && hasTajweed)) {
      clearSurahCacheById(selectedSurah.id);
      setVerses([]);
      setCurrentPage(1);
      setHasMoreVerses(true);
      loadInitialVerses(selectedSurah);
  ...
  useEffect(() => {
    ...
    const sid = paramSurahId ? Number(paramSurahId) : undefined;
    if (!sid || (selectedSurah && selectedSurah.id === sid)) return;
      ...
      <Modal visible={goToModalVisible} transparent animationType="slide" onRequestClose={() => setGoToModalVisible(false)}>
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center', padding: 24 }}>
          <View style={{ width: '100%', maxWidth: 400, backgroundColor: '#222', borderRadius: 16, padding: 24, alignItems: 'center' }}>
            <Text style={{ color: '#FFD700', fontSize: 20, fontWeight: '700', marginBottom: 16 }}>Go to Verse</Text>
            <TextInput
              style={{ width: '100%', color: '#fff', fontSize: 16, backgroundColor: '#333', borderRadius: 8, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 16, borderWidth: 1, borderColor: '#555' }}
              ...
              <Pressable
                ...
                onPress={async () => {
                  ...
                  if (isNaN(num) || num < 1 || num > selectedSurah.versesCount) {
                    setGoToError('Invalid verse number');
                    return;
                  }
                  setGoToSubmitting(true);
                  setGoToError(null);
                  const idx = verses.findIndex((v) => v.verseNumber === num);
                  if (idx !== -1) {
                    try {
                      ...
                      setGoToError('Scroll failed. Try again.');
                    }
                  } else {
                    setGoToError('Verse not loaded yet. Scroll to load more.');
                  }
          ...
          {selectedSurah ? (
            <View style={[styles.headerTitleContainer, { alignItems: 'center' }]}>
              ...
              <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', paddingHorizontal: 8, marginTop: 6 }}>
                ...
                <Text style={[styles.headerSubtitle, { fontFamily: arabicFontFamily, ...arabicTypography, textAlign: 'center', color: '#ffffff' }]}>
...
const styles = StyleSheet.create({
  ...
  searchInputWrapper: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#333333', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 8 },
  searchIcon: { marginRight: 8 },
  searchInput: { flex: 1, fontSize: 16, paddingVertical: 8, color: '#ffffff' },
